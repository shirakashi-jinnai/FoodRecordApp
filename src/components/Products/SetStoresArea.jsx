...
const SetStoresArea = (props) => {
    ...
    const inputStartTime = useCallback(e => {
        setStartTime(e.target.value)
    }, [startTime])

    const inputEndTime = useCallback(e => {
    ...
    const addStores = (place, startTime, endTime, index) => {
        ...
        if (index === props.stores.length) {
            props.setStores(prevState => [...prevState, { place: newPlace, startTime: newStartTime, endTime: newEndTime }]);
        } else {
            const newStores = props.stores
            newStores[index] = { place: newPlace, startTime: newStartTime, endTime: newEndTime }
    ...
    return (
        <TableContainer component={Paper} >
            ...
            <div>
                <TextField
                    value={place} onChange={inputPlace} label='場所' className={classes.place}
                    margin='dense' type='text' placeholder='東京都 足立区' size="small"
                />
                <TextField
                    value={startTime} onChange={inputStartTime} label='開店時刻'
                    margin='dense' type='time' className={classes.time}
                />
                <TextField
                    value={endTime} onChange={inputEndTime} label='閉店時刻'
                    margin='dense' type='time' className={classes.time}
                />
            </div>
